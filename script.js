const certificates = [
    // ── EXAMPLE (replace or add your own below) ──────────────
    {
        ...
        issuer: "Udemy-CodeWithHarry",
        date: "2026",
        category: "Web Development",
        link: "webdevcertificate.pdf"
    },
    {
        icon: "🤖",
...
// ===== HAND-ON SKILLS SECTION — MODAL TOGGLE =====

const hsOpenBtn = document.getElementById('hsOpenBtn');
const hsCloseBtn = document.getElementById('hsCloseBtn');
const fdOverlay = document.getElementById('fdOverlay');

// Open modal
hsOpenBtn.addEventListener('click', function () {
    fdOverlay.classList.add('fd-open');
    document.body.style.overflow = 'hidden'; // prevent background scroll
});

// Close modal via X button
hsCloseBtn.addEventListener('click', function () {
    fdOverlay.classList.remove('fd-open');
    document.body.style.overflow = '';
});

// Close modal by clicking outside the modal box
fdOverlay.addEventListener('click', function (e) {
    if (e.target === fdOverlay) {
        fdOverlay.classList.remove('fd-open');
        document.body.style.overflow = '';
    }
});
...
document.addEventListener('keydown', function (e) {
    if (e.key === 'Escape' && fdOverlay.classList.contains('fd-open')) {
        fdOverlay.classList.remove('fd-open');
        document.body.style.overflow = '';
    }
});
